...
public class CreatureTest {
	...
	public void meetingCreatureShouldUpdateBlindAllayes() {
		...
		assertTrue("creatures should have different maps", creature.getVisitedMap().isUseless(x1+1, y1));
		...
		assertTrue("creatures should be on same square now", movedTo(creature, creature2.getX(), creature2.getY(), 0.5));
		
		assertTrue("creatures should have synchronized USELESS information", creature.getVisitedMap().isUseless(x1+1, y1));
	...
	private static boolean movedTo(Creature c, double newX, double newY, double precision) {
		return newX > c.getX() - precision && newX < c.getX() + precision && newY > c.getY() - precision && newY < c.getY() + precision;
	}
